...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  try {
    ...
    const productResponse = await fetchServer(request, `/api/product/details/${params.id}`)
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  try {
    ...
    const response = await fetchServer(request, `/api/product/edit/${params.id}`, {
      method: "PUT",
  ...
  if (!data.product) {
    return (
      <div className="p-10">
        ...
        <p className="mt-4">The product you're trying to edit could not be found.</p>
      </div>
    )
  }
