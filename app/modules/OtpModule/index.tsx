        ...
        <main className="flex items-center justify-center min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 p-4">
            <Card className="w-full max-w-md shadow-lg border-0">
                <CardHeader className="space-y-1 text-center">
                    ...
                    <h2 className="text-2xl font-bold tracking-tight">OTP Verification</h2>
                    <p className="text-sm text-muted-foreground">Enter the 6-digit code sent to {data.email}</p>
                </CardHeader>
                <CardContent className="space-y-6">
                    ...
                    <Button variant="default" className="w-full" onClick={verifyOtp} disabled={isVerifying || otp.length !== 6}>
                        {isVerifying ? "Verifying..." : "Verify OTP"}
                    </Button>
                </CardContent>
                <CardFooter className="flex flex-col space-y-4">
                    <div className="flex items-center justify-center w-full">
                        <Button variant="outline" className="w-full" onClick={sendOtp} disabled={cooldown > 0 || isSending}>
                            {cooldown > 0 ? (
                    ...
                    <p className="text-xs text-center text-muted-foreground">
                        Didn't receive the code? Check your spam folder or request a new code after the cooldown.
                    </p>
                </CardFooter>
            </Card>
        </main>
