...
export default function VoucherCard({ voucher }: VoucherCardProps) {
  ...
  return (
    <div
      className={`border rounded-lg overflow-hidden shadow-sm ${isExpired ? "opacity-60" : ""}`}
    >
      <div className="bg-blue-100 p-4">
        <h3 className="text-lg font-semibold">Voucher</h3>
        {discount.code && (
          <p className="text-sm font-mono bg-gray-100 p-1 rounded mt-1">
            {discount.code}
          </p>
        )}
      </div>
      ...
      <div className="p-4">
        ...
        {expiredAt && (
          <div className="flex justify-between mb-2">
            ...
            <span
              className={`font-semibold ${isExpired ? "text-red-500" : ""}`}
            >
          ...
          <div>
            {actualVoucher.id && (
              <p className="text-xs text-gray-500">
                ID: {actualVoucher.id.substring(0, 8)}...
              </p>
            )}
          </div>
          {/* <button 
