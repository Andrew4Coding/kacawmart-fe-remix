import { Calendar, ChevronLeft, ChevronRight, User, Wallet } from 'lucide-react';
...
export function WalletTransactions({
    ...
    const walletTransactions = Object.values(transactionData)
        .filter(transaction => transaction.WalletTransaction && transaction.WalletTransaction.length > 0)
                                ...
                                <TableRow key={transaction.id} className="hover:bg-gray-50">
                                    ...
                                    <TableCell>
                                        {transaction.WalletTransaction[0]?.admin ? (
                                            <div className="flex items-center gap-2">
                                                <User size={14} />
                                                <span>{transaction.WalletTransaction[0].admin.userId.substring(0, 10)}...</span>
                                            </div>
                                        ) : (
                                    ...
                                    <TableCell className="text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <Wallet size={14} />
                                            <span>{formatCurrency(transaction.WalletTransaction[0]?.nominal || 0)}</span>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ))
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                    <div className="text-sm text-gray-500">
                        Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, walletTransactions.length)} of {walletTransactions.length} transactions
                    </div>
