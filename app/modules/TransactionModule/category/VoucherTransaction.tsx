...
export function VoucherTransactions({
  transactionData,
}: {
  ...
  const voucherTransactions = Object.values(transactionData)
    .filter(
      (transaction) =>
        ...
        transaction.VoucherTransaction.length > 0,
    )
    .filter(
                ...
                <TableRow key={transaction.id} className="hover:bg-gray-50">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Ticket size={14} />
                      <span>
                        {transaction.VoucherTransaction[0]?.voucherId.substring(
                          0,
                          10,
                        )}
                        ...
                  ...
                  <TableCell>
                    {transaction.VoucherTransaction[0]?.voucher?.discountId.substring(
                      0,
                      10,
                    )}
                    ...
                  ...
                  <TableCell className="text-right">
                    {formatCurrency(
                      transaction.VoucherTransaction[0]?.voucher?.price || 0,
                    )}
                  </TableCell>
                </TableRow>
              ))
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-4">
          <div className="text-sm text-gray-500">
            ...
            {Math.min(currentPage * itemsPerPage, voucherTransactions.length)}{" "}
            of {voucherTransactions.length} transactions
          </div>
