...
export function OrderTransactions({
  ...
  const orderTransactions = Object.values(transactionData)
    .filter(
      (transaction) =>
        transaction.OrderTransaction && transaction.OrderTransaction.length > 0,
    )
    .filter(
                ...
                <>
                  <TableRow key={transaction.id} className="hover:bg-gray-50">
                    ...
                    <TableCell className="text-right">
                      {formatCurrency(
                        calculateTotalPrice(transaction.OrderTransaction[0]),
                      )}
                    </TableCell>
                  </TableRow>

                  {expandedRows[transaction.id] &&
                    transaction.OrderTransaction.map((order, orderIndex) => (
                      <TableRow
                        key={`${transaction.id}-expanded-${orderIndex}`}
                              ...
                              <Table>
                                ...
                                <TableBody>
                                  {order.product?.map((item, itemIndex) => (
                                    <TableRow
                                      key={`${order.id}-product-${itemIndex}`}
                                    >
                                      <TableCell>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-4">
          <div className="text-sm text-gray-500">
            ...
            {Math.min(currentPage * itemsPerPage, orderTransactions.length)} of{" "}
            {orderTransactions.length} transactions
          </div>
