...
const UploadedElement = ({ file }: UploadedElementProps) => {
  return (
    <div className="flex flex-col gap-4 items-center py-6">
      ...
      <p className="text-black text-xs text-center max-w-[100px] truncate">
        {file?.name || "No file name available"}
      </p>
    </div>
  );
};
...
const NotUploadedElement = ({ secondaryMessage }: NotUploadedElementProps) => {
  return (
    <div className="flex flex-col gap-1 sm:gap-4 items-center px-2 sm:px-12 py-6">
      ...
      {secondaryMessage && (
        <p className="text-gray-500 text-xs">{secondaryMessage}</p>
      )}
    </div>
  );
};
    ...
    <div className={cn("flex flex-col gap-2", "w-full")}>
      ...
      <div
        ...
        className={cn(
          ...
          !file ? "bg-white border-[#692597]" : "bg-gray-100 border-gray-500",
          isDragActive ? "active" : "",
        )}
      >
        <input {...getInputProps()} />
